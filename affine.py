...
if __name__ == '__main__':
    ...
    matrix= cv2.getAffineTransform(np.float32(img_src_coordinate[0:3]), np.float32(paste_coordinate))
    ...
    perspective_img = cv2.warpAffine(img_src, matrix, (img_dest.shape[0], img_dest.shape[1]))

    border = [matrix @ np.append(coord, 1) for coord in img_src_coordinate]
    print(border)
    ...
    cv2.fillPoly(mask, [np.int32(border)], (255, 255, 255))
